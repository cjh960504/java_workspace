...
이떄 리스너는 몸체가 불완전한 인터페이스로 지원되는데, 그 이유는 클릭 등을 눌렀을 때
무엇을 어떻게 처리할지의 몫은 개발자가 결정해야 하기 때문이다....
예) 클릭이벤트의 자바에선 액션이벤트라 하고 해당 객체는 ActionEvent이다.
이 ActionEvent의 인스턴스는 사용자가 클릭을 할 때 마다 리스너에게 전달되어지고,
